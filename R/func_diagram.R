...
# program_sel = NULL  -> översikt, andel per programtyp (summerar till 100%).
# program_sel angivet -> programmets andel av SIN programtyp per år.
skapa_diagram_programtyp <- function(df, metrik, metrik_label, program_sel = NULL) {
  df <- dplyr::filter(df, !is.na(programtyp), nzchar(as.character(programtyp)))
  if (nrow(df) == 0) return(.girafe_std(.tom_plot("Inga data"), width_svg = 5, height_svg = 3))

  if (!is.null(program_sel)) {
    typ <- df$programtyp[match(program_sel, df$program)]
    if (is.na(typ)) return(.girafe_std(.tom_plot("Okänd programtyp"), width_svg = 5, height_svg = 3))

    d_typ <- df |> dplyr::filter(programtyp == typ) |>
      dplyr::group_by(ar) |>
      dplyr::summarise(total = sum(.data[[metrik]], na.rm = TRUE), .groups = "drop")
    d_pr <- df |> dplyr::filter(program == program_sel) |>
      dplyr::group_by(ar) |>
      dplyr::summarise(antal = sum(.data[[metrik]], na.rm = TRUE), .groups = "drop")

    d <- dplyr::left_join(d_typ, d_pr, by = "ar") |>
      dplyr::mutate(
        antal   = dplyr::coalesce(antal, 0),
        andel   = ifelse(total > 0, antal / total, 0),
        tooltip = paste0("<b>", program_sel, "</b><br/>", ar, ": ",
                         scales::percent(andel, accuracy = 1), " av ", typ))

    g <- ggplot2::ggplot(d, ggplot2::aes(x = factor(ar), y = andel)) +
      ggiraph::geom_col_interactive(
        ggplot2::aes(tooltip = tooltip, data_id = ar), fill = RD_PRIMARY, width = 0.8) +
      ggplot2::geom_text(ggplot2::aes(label = scales::percent(andel, accuracy = 1)),
                         vjust = -0.4, size = 2.4, color = RD_TEXT_MUTED) +
      ggplot2::scale_x_discrete(breaks = as.character(.ar_breaks(d$ar))) +
      ggplot2::scale_y_continuous(labels = scales::percent,
                                  expand = ggplot2::expansion(mult = c(0, 0.14))) +
      ggplot2::labs(x = NULL, y = NULL, subtitle = paste0("Andel av ", typ)) +
      .rd_tema() +
      ggplot2::theme(plot.subtitle = ggplot2::element_text(size = 9, color = RD_TEXT_MUTED),
                     panel.grid.major.x = ggplot2::element_blank())

    return(.girafe_std(g, width_svg = 5, height_svg = 3, selection = FALSE))
  }
  ...
  d <- df |>
    ...
    dplyr::ungroup() |>
    dplyr::mutate(tooltip = paste0("<b>", programtyp, "</b><br/>", metrik_label, " ", ar, ": ",
                                   scales::percent(andel, accuracy = 1)))

  farger <- programtyp_farger(d$programtyp)

  g <- ggplot2::ggplot(d, ggplot2::aes(x = factor(ar), y = antal, fill = programtyp)) +
    ggiraph::geom_col_interactive(
      ggplot2::aes(tooltip = tooltip, data_id = paste(ar, programtyp)),
      position = "fill", width = 0.8) +
    ggplot2::geom_text(
      ggplot2::aes(label = ifelse(andel >= 0.06, scales::percent(andel, accuracy = 1), "")),
      position = ggplot2::position_fill(vjust = 0.5), size = 2.4, color = "#ffffff") +
